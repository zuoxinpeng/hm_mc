...
/**
 * 消息中心_IMO表（失败） DAO interface 接口
 * 
 * @author 王凯
 * 
 */
@Component
public interface IMcBasImoFailDao extends HibernateDao<McBasImoFail> {

	/**
	 * 根据 <br />
	 * SEND_TIME Time 发送开始时间<br />
	 * 返回表MC_BAS_IMO_FAIL 消息中心_IMO表（失败）的数据列表<br />
	 * 开发者 王凯 2015年01月05日 15:54:34
	 * 
	 * @param sendTimeStart
	 *            用于区间查询的发送开始时间
	 * @param sendTimeEnd
	 ...
	 * @return 数据列表的强类型实体类
	 */
	@Find(" from McBasImoFail where 1=1  #{and createTime >= :sendTimeStart and createTime <= :sendTimeEnd}  #{order by :order :dir}")
	public List<McBasImoFail> findByTime(@Param("sendTimeStart") java.util.Date sendTimeStart, // 查询使用发送开始时间
	 ...
	 * @return 数据列表的强类型实体类
	 */
	@Find(" from McBasImoFail where 1=1  #{and lower(receiver) like :receiver}  #{order by :order :dir}")
	public PageList<McBasImoFail> findByReceiver(@Like("receiver") java.lang.String receiver, // IMO接收人（邮箱的前缀拼音组成）
			@Replace("order") String order, // 排序字段
			@Replace("dir") String orderDir, // 排序顺序
