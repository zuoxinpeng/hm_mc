...
public class McBasLogController {
	...
	public String listWin(@RequestParam(value = "searchContent", required = false) java.lang.String content,// 日志内容
	...
	public String editWin(PageStorage ps, // 页面对象
		...
		if (logId <= 0) { // EDIT_KEY的值<=0表示添加日志表信息
		...
		if (basMcLog == null) { // 数据库中不存在对应的日志表信息，清除EDIT_KEY的值
	...
	public String editBasMcLog(@RequestParam(value = "logId") java.lang.Long logId,// 日志ID
			UserProfile up, // 用户
			PageStorage ps,// 页面对象
			HttpServletRequest request // 请求
	) {
