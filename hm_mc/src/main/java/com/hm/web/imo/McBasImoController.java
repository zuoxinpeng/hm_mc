...
public class McBasImoController {
	...
	public String addBasImo(UserProfile up, // 用户
			PageStorage ps,// 页面对象
			HttpServletRequest request // 请求
	) {
	...
	public String editBasImo(@RequestParam(value = "imoId") java.lang.Long imoId,// IMOID
			UserProfile up, // 用户
			PageStorage ps,// 页面对象
			HttpServletRequest request // 请求
	) {
	...
	public String editWin(PageStorage ps, // 页面对象
		...
		if (id <= 0) { // EDIT_KEY的值<=0表示添加消息中心_IMO表信息
		...
		if (basImo == null) { // 数据库中不存在对应的消息中心_IMO表信息，清除EDIT_KEY的值
	...
	// 验证
	@Validates({ @Validate(name = "receiver", id = "ne", error = "IMO接收人不能为空。"),// IMO接收人（邮箱的前缀拼音组成）^[A-Za-z]+$
	...
	public String save(@Valid McBasImo basImo, // 保存校验的对象
		...
		for (int i = 0; i < str.length; i++) {
			...
			if (jsonObj.has("uid")) {
				...
				McBasImoFail mcBasImoFail = new McBasImoFail(str[i], "0", basImo.getSubject(), basImo.getContent(), basImo.getKey(), 2L, basImo.getGrpId(), "IMO接收人不存在", basImo.getSendTime(), new Date(), null);
