...
public class McBasEmailSuccessController {
	...
	public String listWin(
			@RequestParam(value = "searchReceiver", required = false) java.lang.String receiver,// 邮件接收人
	...
	public String editBasMcEmailSuccess(
			@RequestParam(value = "maileId") java.lang.Long maileId,// 短信ID
			UserProfile up, // 用户
			PageStorage ps,// 页面对象
			HttpServletRequest request // 请求
	)
	...
	)
	{
		...
		if (maileId <= 0)
		{ // EDIT_KEY的值<=0表示添加消息中心_邮件表（成功）信息
		...
		if (basMcEmailSuccess == null)
		{ // 数据库中不存在对应的消息中心_邮件表（成功）信息，清除EDIT_KEY的值
