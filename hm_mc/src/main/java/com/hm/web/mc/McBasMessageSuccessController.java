...
public class McBasMessageSuccessController {
	...
	public String editWin(PageStorage ps, // 页面对象
		...
		if (id <= 0) { // EDIT_KEY的值<=0表示添加消息中心_短信表（成功）信息
		...
		if (basMcMessageSuccess == null) { // 数据库中不存在对应的消息中心_短信表（成功）信息，清除EDIT_KEY的值
	...
	public String editBasMcMessageSuccess(@RequestParam(value = "messageId") java.lang.Long messageId,// 短信ID
			UserProfile up, // 用户
			PageStorage ps,// 页面对象
			HttpServletRequest request // 请求
	) {
