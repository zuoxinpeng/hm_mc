...
public class McTemplateController {
	...
	public String delMcTemplate(
			@RequestParam(value = "templateId") java.lang.Long templateId,// 模板主键
			UserProfile up, // 用户
	...
	public String editMcTemplateDetail(
			@RequestParam(value = "templateId") java.lang.Long templateId,
			@RequestParam(value = "templateTypeId") java.lang.Long templateTypeId,
			Model model, HttpSession ps) {
	...
	@Window
	@Return({
			@Code(value = "", log = "添加/编辑窗口不显示"),// 提示信息
	...
	public String editWin(@PageRefresh boolean refresh, HttpSession ps,Model model) {
		...
		if (ps.getAttribute(EDIT_KEY) != null
				&& ps.getAttribute(EDIT_KEY).equals("add")) {
	...
	@Return({ @Code(value = "", log = "保存模板操作成功", msg = "添加模板成功", update = "listWin"),// 提示信息
			@Code(value = "repeat", log = "模板已存在,请重新添加",  msg = "模板已存在,请重新添加",view = "") ,
			@Code(value = "vf", log = "AJAX表单字段校验", view = ""),
			@Code(value = "error", log = "表单字段校验出错", view = "") })// 提示信息
	// 验证
	@Validates({
			@Validate(name = "templateEncode", id = "ne", error = "模板编码不能为空。"),// 模板编码
	...
	// 验证
	@Validates({
		    @Validate(name = "templateTitle", id = "ne", error = "模板标题不能为空。"),// 模板标题
