 ...
 */
@Entity
@Table(name = "ZT_MC_BAS_IMO_SUCCESS")
public class McBasImoSuccess implements Serializable {
	/**
	 * Java的序列化机制是通过在运行时判断类的serialVersionUID来验证版本一致性的。在进行反序列化时，JVM会把传来的字
	 * 节流中的serialVersionUID与本地相应实体（类）的serialVersionUID进行比较，如果相同就认为是一致的，可以进行反
	 * 序列化，否则就会出现序列化版本不一致的异常
	 */
	private static final long serialVersionUID = 2015010514192779393L;

	public McBasImoSuccess() {
	}

	public McBasImoSuccess(String receiver, String receiverUid, String subject, String content, String key, long status, String grpId, Date sendTime, Date createTime, Date updateTime) {
		super();
	...
	@SequenceGenerator(name = "generator", sequenceName = "SEQ_ZT_MC_BAS_IMO_SUCCESS", allocationSize = 1)
