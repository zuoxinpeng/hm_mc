...
@Service
@Transactional
public class McTemplateDetailService extends
		HibernateServiceBase<McTemplateDetail> implements
		IMcTemplateDetailService {
	@Autowired
	...
	public List<McTemplateDetail> findByTemplateCode(String templateCode) {
		...
		List<McTemplate> mcTemplateList = mcTemplateService
				.findByTemplateEncode(templateCode);
		...
		if (mcTemplateList != null && mcTemplateList.size() > 0) {
			...
			mcTemplateDetailList = theDao.findByTemplateId(templateId) == null ? null
					: theDao.findByTemplateId(templateId);
		}
	 ...
	 * @return McTemplateDetail对象实体
	 */
	public McTemplateDetail getByTemplateEncodeAndTypeId(String templateEncode,
			java.lang.Long templateTypeId) {
		return theDao.getByTemplateEncodeAndTypeId(templateEncode,
				templateTypeId);
	}
