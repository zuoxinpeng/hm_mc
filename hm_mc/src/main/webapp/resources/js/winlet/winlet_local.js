	...
	return this.each(function() {
		...
		this.winsubmit = function() {
			if ($(this).attr("enctype") == "multipart/form-data") {
				this.action = this.action + "&" + AeJSEngine.mergeParam(settings.iid, {_pg: window.location.pathname, _purl: window.location.href});
				return true;
			}
			...
			if ($(this).attr('method').toUpperCase() != 'POST') { // get方式无需调用action，直接刷新window
				...
				try {
					if ($(this).attr('hideloading') != 'yes') {
						if (settings.iid == settings.vid)
							AeJSEngine.showLoading('#ap_win_' + settings.iid);
						else
							AeJSEngine.showLoading('#ap_view_' + settings.vid);
					}
				} catch (e) {
				}
				...
				$.ajax({
					  type: 'POST',
					  url: this.action,
					  data: AeJSEngine.mergeParam(settings.iid, $.deparam($(this).serialize()), {_x: 'y', _v: settings.validate, _ff: fields, _fd: disabled, _pg: window.location.pathname, _purl: window.location.href}),
					  success: AeJSEngine.getSubmitFormHandler(settings.iid, settings.vid, this),
					  error: AeJSEngine.getErrorHandler(settings.iid, settings.vid),
					  dataType: "text"
					});
			}
		...
		this.ajaxValidate = function(input, name, value) {
			...
			AeJSEngine.validating($(input.form).find("span#validate_res_" + name.replace("[", "\\[").replace("]", "\\]").replace(".", "\\."))[0]);
			...
			$.ajax({
				  type: 'POST',
				  url: this.action,
				  data: AeJSEngine.mergeParam(settings.iid, param, {_x: 'y', _pg: window.location.pathname, _purl: window.location.href}),
				  success: AeJSEngine.getValidateResponseHandler(input.form, name, input),
				  dataType: "json"
				});
		};
		...
		if (settings.focus)
			$(this).find('input[name="' + settings.focus + '"]').focus();
		...
		this.updateValidate = function() {
			if (settings.validate == 'yes') {
				...
				$(this).find(":input").each(function() {
					...
					this.m_result = $(form).find("span#validate_res_" + this.name.replace("[", "\\[").replace("]", "\\]").replace(".", "\\."));
					if (this.m_result.length == 0) {
						this.m_result = $('<span id="validate_res_' + this.name + '"></span>');
						$(this).after(this.m_result);
					}

					if ($(this).attr("tips") != undefined) {
						$(this).focus(function(){
							if (this.m_result.html() == '') {
								this.m_result.html('<div class="win_tips">' + $(this).attr('tips') + '</div>');
							}
						});
						$(this).blur(function(){
							if ($(this.m_result).find("div.win_tips").length > 0) {
								this.m_result.html('');
							}
						});
					}
...
var AeJSEngine = {
	...
	reWinlet: new RegExp('^winlet:((\\w+/.+)/(\\w+))(\\?([^\\s]+))?\\s*(\\((.*?)\\))?$'),
	...
	getHash: function(iid) {
		...
		try {
			return $.param($.deparam(window.location.hash.slice(1))[hashgroup], true);
		} catch (e){
		}
		return "";
	},
	...
	showLoading: function(id, visible) {
		try {
			...
			var rect = new ElmRect($(id));
			try {
				if (AeJSEngine.dlg != null && AeJSEngine.dlg.dialog('isOpen') === true)
			...
			if (visible) {
				...
				html = "<div id='" + id.substr(1) + "_loading' style='z-index:100000;position:absolute;left:" + rect.left + "px;top:" + rect.top
				+ "px;width:" + rect.width + "px;height:" + rect.height
				+ "px'></div>";
			}
			$("body").append(html);
...
//			elmHead.appendChild(newScript);
		...
		if(q){
			AeJSEngine.loadScript(list_q).done(function() {
					for (i = 0; i < scriptContent.length; i++)
						try {
							eval(scriptContent[i]
								.replace(AeJSEngine.reWinPost, 'win$._post(' + iid + ', ' + vid + ', ')
								.replace(AeJSEngine.reWinGet, 'win$._get(' + iid + ', ' + vid + ', ')
								.replace(AeJSEngine.reWinUrl, 'win$._url(' + iid + ', ' + vid + ', ')
								.replace(AeJSEngine.reWinSubmit, 'win$._submit(' + iid + ', ' + vid + ', ')
								);
						} catch (e) {
							alert(e);
	...
	getLoadContentHandler: function(iid, vid) {
		return function (data, textStatus, jqXHR) {
				...
				if (settings != null) {
					if (settings.dialog == "yes") {
						...
						if (container.length == 0) {
							container = $("<div id='ap_win_" + iid + "_dialog'></div>");
							isnew = true;
						}
						else {
							try {
						...
						container.dialog({
							...
							close: function(event, ui) {
								if (settings.close != null) {
									$.ajax({
										  type: 'POST',
										  url: settings.url + "?_a=" + iid + "!" + vid + "!" + settings.close,
										  data: AeJSEngine.mergeParam(iid, $.deparam($(this).serialize()), {_x: 'y', _pg: window.location.pathname, _purl: window.location.href}),
			...
			//如果有遮罩层，需要关闭
			//用来解决请求后台前弹出遮罩层，后台处理之后无法关闭遮罩层的问题
			//此处是解决后台返回错误信息时，遮罩层不消失的问题
			//add by suicl 2013-11-28
			if($(".trd-loading-div").length > 0){
				$(".trd-loading-div").hide();
			}
			
			AeJSEngine.procScript(iid, vid, data);
			AeJSEngine.invokeAfterLoad();
			AeJSEngine.clearLoading(uid);
			if(typeof(Fps) != "undefined" &&document.getElementsByName('FpsSmallIconUser').length!=0){
	...
	loadContent: function(iid, vid, pageRefresh) {
		...
		$.ajax({
			  ...
			  data: AeJSEngine.mergeParam(iid, {_x: 'y', _w: iid, _wv: vid, _pg: window.location.pathname, _purl: window.location.href, _pr: pageRefresh ? "yes" : "no"}),
	...
	applyChanges: function(json, form) {
		...
		if (changes != null) {
			for (var i = 0; i < changes.length; i++) {
				var result = $(form).find("span#validate_res_" + changes[i].input.replace("[", "\\[").replace("]", "\\]").replace(".", "\\."))[0];
				var inp = $(form).find(":input[name='" + changes[i].input + "']")[0];
	...
	getSubmitFormHandler: function(iid, vid, form) {
		return function (data, textStatus, jqXHR) {
			...
			var title = AeJSEngine._utf8_decode(jqXHR.getResponseHeader('X-Winlet-Title'));
			var msg = AeJSEngine._utf8_decode(jqXHR.getResponseHeader('X-Winlet-Msg'));
			// var ensurevisible = jqXHR.getResponseHeader('X-Winlet-EnsureVisible');
	...
	getValidateResponseHandler: function(form, name, input) {
		return function(json) {
			AeJSEngine.validate_clear($(form).find("span#validate_res_" + name.replace("[", "\\[").replace("]", "\\]").replace(".", "\\."))[0]);
	 ...
	 ******************************************************************************/
	init: function() {
		AeJSEngine.ImgBg.src = $("<div class='winlet_background'></div>").css('background-image').replace(/^url|[\(\)"]/g, ''); 
		AeJSEngine.ImgLoading.src = $("<div class='winlet_loading'></div>").css('background-image').replace(/^url|[\(\)"]/g, ''); 
		AeJSEngine.ImgValidating.src = $("<div class='winlet_validating'></div>").css('background-image').replace(/^url|[\(\)"]/g, ''); 

		AeJSEngine.isStatic = true;
		$('div').filter(function() {return this.id.match(AeJSEngine.reWinlet);}).each(function(idx) {
...
var win$ = {
	...
	_post: function(iid, vid, action) {
		...
		$.ajax({
			type: 'POST',
			url: AeJSEngine.getWinSettings(iid).url,
			data: AeJSEngine.mergeParam(iid, params, {_x: 'y', _w: iid, _wv: vid, _a: action, _pg: window.location.pathname, _purl: window.location.href}),
			success: AeJSEngine.getSubmitFormHandler(iid, vid),
			error: AeJSEngine.getErrorHandler(iid, vid),
			dataType: "html"
		});
	...
	_url: function(iid, vid, action) {
		...
		return AeJSEngine.getWinSettings(iid).url + "?" + AeJSEngine.mergeParam(iid, params, {_x: 'y', _w: iid, _wv: vid, _a: action, _pg: window.location.pathname, _purl: window.location.href});
	},
	...
	_submit: function(iid, vid, form, action) {
		...
		if (action != null && action != '') {
			if (!f.attr('action').match(win$.reAction))
				return;
			f.attr('action', f.attr('action').replace(win$.reAction, "$1!$2!" + action));
		}
