...
public class McBasImoFailController {
	...
	public String listWin(
			@RequestParam(value = "searchReceiver", required = false) java.lang.String Receiver,// IMO接收人
	...
	)
	{
		...
		if (id <= 0)
		{ // EDIT_KEY的值<=0表示添加消息中心_IMO表（失败）信息
		...
		if (basImoFail == null)
		{ // 数据库中不存在对应的消息中心_IMO表（失败）信息，清除EDIT_KEY的值
